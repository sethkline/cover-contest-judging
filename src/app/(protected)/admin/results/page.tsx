  ...
  const contestResults = await Promise.all(
    contests?.map(async (contest) => {
      ...
      const judgeStats = judges?.map(judge => {
        ...
        const completionRate = totalEntries ? Math.round((entriesJudged / totalEntries) * 100) : 0;
      ...
      const completedJudges = judgeStats?.filter(j => j.isComplete).length || 0;
      const overallCompletionRate = totalJudges ? Math.round((completedJudges / totalJudges) * 100) : 0;
    ...
    <div className="space-y-6">
      ...
      {contestResults.length === 0 ? (
        <div className="bg-white p-8 rounded-lg shadow text-center">
          <h2 className="text-xl font-medium text-gray-700">No contests found</h2>
          <p className="text-gray-500 mt-2">Create contests to see results here.</p>
        </div>
      ) : (
        <div className="space-y-8">
          {contestResults.map(contest => (
            <div key={contest.id} className="bg-white rounded-lg shadow overflow-hidden">
              <div className="p-6 border-b">
                ...
                <div className="mt-2 grid grid-cols-3 gap-4 text-sm text-gray-600">
                  <div>
                    <span className="font-medium">Entries:</span> {contest.totalEntries}
                  </div>
                  <div>
                    <span className="font-medium">Judges:</span> {contest.totalJudges}
                  </div>
                  <div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {contest.judgeStats?.map(judge => (
                        <tr key={judge.id}>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{judge.email}</td>
                            ...
                            <div className="w-full bg-gray-200 rounded-full h-2.5">
                              <div 
                                className={`h-2.5 rounded-full ${
                                  judge.isComplete ? 'bg-green-600' : 'bg-blue-600'
                                }`}
