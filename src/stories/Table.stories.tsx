      ...
      <Table variant="bordered" className="w-full max-w-3xl">
        ...
        <TableHeader>
          <TableRow>
            <TableHead 
              isSortable 
              sortDirection={sortConfig.key === 'entryNumber' ? sortConfig.direction : null}
            ...
            <TableHead 
              isSortable 
              sortDirection={sortConfig.key === 'participant' ? sortConfig.direction : null}
            ...
            <TableHead 
              isSortable 
              sortDirection={sortConfig.key === 'averageScore' ? sortConfig.direction : null}
        ...
        <div className="flex justify-between items-center">
          <div>
            {selectedRows.length > 0 && (
              <span className="text-sm">{selectedRows.length} entries selected</span>
            )}
          </div>
                ...
                <div className="flex-1 mr-2">
                  <div className="bg-neutral-200 dark:bg-neutral-700 h-2 rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ width: `${(judge.entriesJudged / judge.totalEntries) * 100}%` }}
                    />
                  </div>
                </div>
      ...
      <TableBody>
        <TableEmptyState colSpan={6}>
          ...
          <p className="text-sm text-neutral-500 max-w-xs mb-4">
            There are no entries available for this contest yet. Entries will appear here once they are submitted.
          </p>
...
function StatusBadge({ status, size = 'default' }: { status: string, size?: 'sm' | 'default' | 'lg' }) {
  switch (status) {
    case 'Scored':
      return (
        <Badge variant="success" size={size} icon={<Check size={size === 'sm' ? 10 : 12} />}>
          Scored
        </Badge>
      );
    case 'Pending':
      return (
        <Badge variant="warning" size={size} icon={<AlertCircle size={size === 'sm' ? 10 : 12} />}>
          Pending
        </Badge>
      );
    case 'In Review':
