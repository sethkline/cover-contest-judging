...
const JudgeDashboard = () => {
    ...
    async function fetchJudgeData() {
      try {
        ...
        for (const contest of contests) {
          ...
          if (entriesError) {
            console.error(
              `Error fetching entries for contest ${contest.id}:`,
              entriesError,
            );
            throw entriesError;
          }
          ...
          console.log(
            `Entries found for contest ${contest.name}:`,
            entriesData.length,
          );
          ...
          if (entriesData.length > 0) {
            console.log(
              "Entry category IDs:",
              entriesData.map((e) => e.age_category_id),
            );
          }
          ...
          if (scoresError) {
            console.error(
              `Error fetching scores for user ${user.id}:`,
              scoresError,
            );
            throw scoresError;
          }
  ...
  const totalJudged = Object.values(judgingProgress).reduce(
    (total, contest) =>
      total +
      contest.categories.reduce((catTotal, cat) => catTotal + cat.judged, 0),
    0,
  );
  ...
  const totalEntries = Object.values(judgingProgress).reduce(
    (total, contest) =>
      total +
      contest.categories.reduce((catTotal, cat) => catTotal + cat.total, 0),
    0,
  );
