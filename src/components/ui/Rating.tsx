...
export function Rating({
  ...
  return (
    <div className={cn("flex flex-col", className)}>
      ...
      <div className={cn("relative flex items-center", containerSizes[size])}>
        <input
          type="range"
          min={0}
          max={max}
          value={value}
          onChange={(e) =>
            !disabled && onChange && onChange(Number(e.target.value))
          }
          className={cn(
          ...
          style={{
            ...
            "--webkit-slider-thumb-width":
              size === "sm" ? "0.75rem" : size === "lg" ? "1.25rem" : "1rem",
            "--webkit-slider-thumb-height":
              size === "sm" ? "0.75rem" : size === "lg" ? "1.25rem" : "1rem",
          }}
          disabled={disabled}
